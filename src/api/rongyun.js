  ...
  if (message.targetId === _this.consults.groupid) {
    ...
    // 4. 医生开出个人处方 / 无库存处方消息
      ...
      data = {
        ...
        description: content.description ? content.description : content.message,
        orderId: content.orderId ? content.orderId : content.order_id,
        status: content.status,
        sendType: content.type ? content.type : ''
      };
      // _this.sendChat(data);
      // _this.scrollToBottom();
    }
    ...
    if (sentTime) {
      // 间隔等于或超过5分钟设置一次时间
      if (Math.abs(message.sentTime - sentTime) >= 1000 * 60 * 5) {
        sessionStorage.setItem('inquiryMessageSentTime', message.sentTime);
