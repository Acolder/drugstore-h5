  ...
  methods: {
    ...
    fenxiang() {
      ...
      if (_self.isWX) {
        ...
        MessageBox('提示', '请使用浏览器自带的分享功能发送给朋友吧');
      }
      let title = '';
      ...
      _self.wx.ready(function() {
        const share = {
          title: title, // 分享标题
          desc: '开心！我领取到了一份康美人生精美礼品，快来体验吧~', // 分享描述
      ...
      let qqshare = {
        title: title, // 分享标题
        summary: '开心！我领取到了一份康美人生精美礼品，快来体验吧~', // 分享内容
        pic: _self.detailData.image1, // 分享图片
        url: `${process.env.WEB_HOST}/gift/receive?orderId=${_self.detailData.orderId}`, // 分享链接
      };
    ...
    giving() {
      ...
      _self.wx.ready(function() {
        const share = {
          title: title, // 分享标题
          desc: '您的好友给您送了一份康美人生精美礼品，快来领取吧', // 分享描述
          link: `${process.env.WEB_HOST}/gift/receive?orderId=${_self.detailData.orderId}`, // 分享链接
          imgUrl: _self.detailData.image1, // 分享图标
          success: function (e) {
          // 用户确认分享后执行的回调函数
      ...
      let qqshare = {
        title: title, // 分享标题
        summary: '您的好友给您送了一份康美人生精美礼品，快来领取吧', // 分享内容
        pic: _self.detailData.image1, // 分享图片
        url: `${process.env.WEB_HOST}/gift/receive?orderId=${_self.detailData.orderId}`, // 分享链接
      };
      mqq.data.setShareInfo(qqshare, function(e) {
