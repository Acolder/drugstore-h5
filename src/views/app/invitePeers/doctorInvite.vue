        ...
        <div class="content-bg">
          ...
          <div class="cell">
            ...
            <div class="cell-bd select-bd">
              <select class="cell-select" v-model="hospitalSelect" :class="{ selected: hospitalChange }" @change="hospitalChange = selectChange(hospitalChange)">
                <option value="" disabled="disabled" selected="selected">请选择医院</option>
                <option v-for="hos in hospital" :key="hos.id" :value="hos">{{ hos.hosName }}</option>
              </select>
            </div>
          </div>
          <div class="cell">
            ...
            <div class="cell-bd select-bd">
              <select class="cell-select" v-model="departSelect" :class="{ selected: departChange }" @change="departChange = selectChange(departChange)">
                <option value="" disabled="disabled" selected="selected">请选择科室</option>
                <option v-for="dep in depart" :key="dep.departId" :value="dep.departId">{{ dep.departName }}</option>
              </select>
            </div>
          </div>
          <div class="cell">
          ...
          <div class="cell">
            ...
            <div class="cell-bd">
              <span class="cell-bd-tips">(图片需包含身份证和执业医师证号)</span>
            </div>
          </div>
          <div class="cell">
            <div class="cell-hd">
...
export default {
  ...
  data() {
    return {
      ...
      depart: [], // 科室
      jobTitle: [], // 职称
      hospitalSelect: '', // 所属系统默认选项
      departSelect: '', // 职称默认选项
      titleSelect: '', // 职称默认选项
      sex: '', // 性别
      hospitalChange: false, // 医院选项改变
      departChange: false, // 科室改变
  ...
  methods: {
    ...
    getDepartList() {
      let _this = this;
      docAjax.get(`/api/doctor/getDepartList`).then(resp => {
        console.log(resp);
        if (resp.status === 200 && resp.data.code === 0) {
          let respData = resp.data.data;
          respData.forEach(value => {
    ...
    submit() {
      ...
      if (_this.hospitalSelect.length === 0) {
        Toast('请选择医院！');
        return false;
      }
      if (_this.departSelect.length === 0) {
        Toast('请选择科室！');
        return false;
      }
    ...
    toSubmit() {
      ...
      let data = {
        ...
        DoctorType: 0, // 医生类型 0-医师 1-药师
        real_name: _this.username, // 姓名
        refer_hos_name: _this.hospitalSelect.hosName, // 所属医院名称（系统名称）
        hos_id: _this.hospitalSelect.id, // 所属医院id（系统id）
        depart_id: _this.departSelect, // 科室id
