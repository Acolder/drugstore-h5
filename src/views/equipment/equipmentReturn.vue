<template>
<div class="equipmentReturn">
  <mt-header title="归还脉诊仪">
    <mt-button icon="back" slot="left" onclick="history.go(-1)"></mt-button>
    <!-- <mt-button icon="back" slot="left" @click="back"></mt-button> -->
  </mt-header>
  <div class="equipmentReturnMain" >
    <div class="login" v-if="login">
      <div @click="gologin">未登录</div>
    </div>
    <div v-else class="login-data">
      <div v-if="listData.length>0">
        <div class="list-item">请选择归还批次</div>
        <div class="list-item" v-for="item in listData" :key='item.index' @click="details(item)">
          ...
          <div class="txtgreen" v-if="item.order_status === 1">待收货</div>
          <div class="txtgreen" v-if="item.order_status === 2">已发货</div>
...
export default {
  ...
  methods: {
    ...
    details(item) {
      ...
      this.$router.push('/equipment/equipmentReturnDetails?user=' + user + '&id=' + id);
