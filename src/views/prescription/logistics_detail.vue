<template>
  <div class="page">
    <div>
      ...
      <div class="logisticsCompany" v-if="this.logisticsInfo.company.logis_company&&this.logisticsInfo.company.logis_num">
        <div>
          <span>物流公司：</span>
          <span>{{logisticsInfo.company.logis_company}}</span>
        </div>
        <div>
          <span>运单号：</span>
          <span>{{logisticsInfo.company.logis_num}}</span>
        </div>
      </div>
      <div class="logisticsInformation">
        <ul class="logistics-wrapper" v-if="this.logisticsInfo.list&&this.logisticsInfo.list.length>0">
          <li class="logistics-item" v-for="item in logisticsInfo.list" :key="item.id">
            <div class="item-left">
              <i class="logistics-signal"></i>
              <div class="ftime">{{item.current_lo_time || item.AcceptTime}}</div>
            </div>
            <div class="item-right">
              {{item.addr_info || item.AcceptStation}}
            </div>
          </li>
        </ul>
