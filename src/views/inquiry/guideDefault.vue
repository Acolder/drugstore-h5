<template>
  <div id="guideDefault">
    ...
    <div class="inquiry-page has-status-bar" id="inquiry-page" style="height: 500px;overflow: scroll">
      <me-chat v-for="(object) in chatArrays" :name='object.name' :message='object.message'
               :actor='object.actor' :key='object.id'>
      </me-chat>
      <div v-if="hasChecked">
        <me-chat name="导诊医生" message="就诊信息已提交，这是您的就诊信息：" actor="doctor" :avatar='this.user.photopath'>
        ...
        <me-chat name="导诊医生" message="正根据您的症状为您推荐医生，请稍后..." actor='doctor'>
        </me-chat>
        <!-- <me-chat name="导诊医生" v-if="hasChecked" :message="doctorMsg" actor='doctor'>
        ...
        <me-chat name="导诊医生" v-if="hasChecked" message="医生正在问候其他患者，您可先进入候诊室接待。" actor='doctor'>
      ...
      <me-chat name="导诊医生" v-if="hasChecked" :message="`为您推荐了1位医生，请选择医生进行咨询`" actor='doctor'><!-- ${consultsArray.length} -->
      </me-chat>
    </div>
...
export default {
  ...
  methods: {
    ...
    doctorDetail(id) {
      ...
      let memberid = this.patient.memberid; // 就诊人ID
      let description = this.patient.description; // 病情描述
      let name = this.patient.name; // 就诊人姓名
      let gender = this.patient.gender; // 就诊人性别
      let age = this.patient.age; // 就诊人年龄
      let mobile = this.patient.mobile; // 就诊人手机号
