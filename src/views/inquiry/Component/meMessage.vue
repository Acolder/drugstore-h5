<template>
  <div class="chat-box " v-bind:class="[header.direction, message.content.messageName ==='UnknownMessage'&&message.content.message.objectName==='app:recommendDrug'? 'whiteChat': '']">
    <!--<div>-->
      <!--{{message.content.messageName + '分割线' + JSON.stringify(message.content.message)}}-->
      <!--<br>-->
    <!--</div>-->
    <!-- 头像-->
    ...
    <div class="speech" v-if="visible">
      <div class="angel"></div>
      <div class="info" :class="message.content.messageName ==='UnknownMessage'&&message.content.message.objectName==='app:recommendDrug'? 'whiteChat-Box': ''">
        <!--如果文本文件-->
        ...
        <slot></slot>
        <!--如果是商品链接-->
        <div v-if="message.content.messageName ==='UnknownMessage'&&message.content.message.objectName==='app:recommendDrug'">
          <div class="User-drugsCommend-container">
            <a :href="'/goods/goodsDetail?productId='+reportDetail.prodId+'&amp;getByself='+reportDetail.getByself+'&amp;getCommon='+reportDetail.getCommon+'&amp;getFast='+reportDetail.getFast">
              <div class="top" style="flex-shrink: 0">
                <div class="box-left">
                  <img :src="reportDetail.prodPhotoPath" alt="图片" style="width: 100%;border: 1px solid #CCC;border-radius:3px">
                </div>
                <div class="box-right">
                  <div class="introduce sigleTextOverflow">{{reportDetail.prodName}}{{reportDetail.description}} </div>
                  <div class="price">¥{{reportDetail.price}}</div>
                </div>
              </div>
            </a>
          </div>
...
export default {
  ...
  computed: {
    ...
    reportDetail: function () {
      let reportDetail;
      if (typeof (this.message.content.message.content.content) === 'string') {
        reportDetail = JSON.parse(this.message.content.message.content.content);
      } else {
        reportDetail = this.message.content.message.content.content;
      }
      return reportDetail;
    },
    visible: function () {
      if (this.message.content.messageName === 'UnknownMessage') {
        console.log(this.message, this.message.content.message.objectName);
        if (this.message.content.message.objectName === 'app:dialecticalPresc' || this.message.content.message.objectName === 'app:recommendDrug') {
          return true;
        } else {
          return false;
        }
...
<style lang="less" scoped>
  ...
  .chat-box.right-chat.whiteChat .speech .angel{
